...
class TestAgentPrompts:
    ...
    def test_all_nine_agents_present(self):
        ...
        expected_ids = {"general", "health", "finance", "legal", "career",
    ...
    def test_every_agent_has_non_empty_system_prompt(self):
        ...
        for agent_id, config in AGENTS.items():
            assert config.system_prompt.strip(), (
                f"Agent '{agent_id}' has an empty system prompt."
            )
    ...
    def test_disclaimer_agents_have_disclaimer(self):
        ...
        for agent_id in ("health", "finance", "legal"):
            assert AGENTS[agent_id].disclaimer, (
                f"Agent '{agent_id}' must have a disclaimer."
            )
...
class TestAgentRouter:
    ...
    def test_get_all_nine_agents(self):
        from assistant.agents.router import AgentRouter
        router = AgentRouter()
        for agent_id in ("general", "health", "finance", "legal",
                         "career", "tutor", "travel", "tech", "creative"):
...
class TestConversationStore:
    ...
    def test_agents_have_isolated_histories(self):
        ...
        store.chat(session_id="sess-1", agent_id="general",
                   system_prompt="General prompt.", message="Hello general")
        store.chat(session_id="sess-1", agent_id="general",
                   system_prompt="General prompt.", message="How are you?")
        ...
        store.chat(session_id="sess-1", agent_id="health",
                   system_prompt="Health prompt.", message="Tell me about sleep")
        health_count = store.message_count(session_id="sess-1", agent_id="health")
    ...
    def test_sessions_have_isolated_histories(self):
        ...
        store.chat(session_id="sess-A", agent_id="general",
                   system_prompt="Prompt.", message="Message from session A")
    ...
    def test_clear_resets_one_agent(self):
        ...
        store.chat(session_id="sess-1", agent_id="general",
                   system_prompt="Prompt.", message="Hello")
        store.chat(session_id="sess-1", agent_id="health",
                   system_prompt="Health.", message="Hi")
    ...
    def test_clear_session_removes_all_agents(self):
        ...
        store.chat(session_id="sess-1", agent_id="general",
                   system_prompt="General.", message="Hi")
        store.chat(session_id="sess-1", agent_id="finance",
                   system_prompt="Finance.", message="Budget help")
    ...
    def test_session_count_tracks_active_pairs(self):
        ...
        store.chat(session_id="s1", agent_id="general",
                   system_prompt="P.", message="Hi")
        assert store.session_count() == 1
        store.chat(session_id="s1", agent_id="health",
                   system_prompt="H.", message="Hi")
...
class TestChatEndpoint:
    ...
    def test_chat_reuses_provided_session_id(self, api_client):
        ...
        resp = tc.post("/chat", json={
            "message": "Hello", "agent_id": "general", "session_id": session_id
        })
    ...
    def test_chat_conversation_history_grows(self, api_client):
        ...
        resp1 = tc.post("/chat", json={
            "message": "Hello", "agent_id": "general", "session_id": session_id
        })
        resp2 = tc.post("/chat", json={
            "message": "How are you?", "agent_id": "general", "session_id": session_id
        })
    ...
    def test_clear_chat_resets_history(self, api_client):
        ...
        resp = tc.post("/chat", json={
            "message": "Fresh start", "agent_id": "general", "session_id": session_id
        })
